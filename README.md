...
`select ename,sal,from emp where sal > min(sal); 	 ` 为什么报错

因为分组函数在使用的时候必须先分组之后才能使用，where 执行的时候还没有分组，所以 where 后面不能出现分组函数



`select min(sal) from emp;` 为什么正确

因为 select 在 分组函数之后执行
...
<img src="C:\Users\CSQ-PC\AppData\Roaming\Typora\typora-user-images\image-20220711220117965.png" alt="image-20220711220117965" style="zoom: 50%;" />	 



`select ename ,job ,sum(sal) from emp group by job;`

以上语句在 MySQL 可以执行但是毫无意义 
...
在一条 select 语句中，如果有 group by 语句的话，select 语句之后只能跟 ： **参加分组的字段，以及分组函数** ，其他的一律不跟  
...
##### 找出每个部门的最高薪资，要求显示最高薪资大于300的

###### having 

使用 having 可以对分组之后的数据进一步过滤，having 不能单独使用 ，having 不能代替 where，having 必须和 group by 联合使用

`select deptno,job,max(sal) from emp group by deptno,job having max(sal) > 3000;`



以上效率较低，可以这样考虑：先将大于 3000 的都找出来，然后再分组

`select max(sal) from emp where sal > 3000 group by deptno,job; `



##### 优化策略：

where 和 having ,优先选择 where ，where  选择不了再选择 having
...
找出每个岗位的平均薪资，要求显示平均薪资大于1500 的，除 MANAGER 之外，要求按 
